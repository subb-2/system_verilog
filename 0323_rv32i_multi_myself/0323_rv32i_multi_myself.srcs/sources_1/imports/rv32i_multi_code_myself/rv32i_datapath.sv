...
module rv32i_datapath (
    ...
    //Write Back
    logic [31:0] o_wb_pc_alu_imm, o_wb_imm, o_wb_pc_alu_4;
    ...
    state_register U_EXE_ALU_RESULT (
        ...
        .rst(rst),
        .in (alu_result),
        .out(o_exe_alu_result)  //to Daddr
    );

    state_register U_EXE_REG_RS2 (
        .clk(clk),
        .rst(rst),
        .in (o_dec_rs2),  // from alu_result
        .out(o_exe_rs2)   // to Data MEM_Wdata
    );
    ...
    state_register U_WB_REG_PC_IMM (
        .clk(clk),
        .rst(rst),
        .in (pc_alu_imm),
        .out(o_wb_pc_alu_imm)
    );
    state_register U_WB_REG_IMM (
        .clk(clk),
        .rst(rst),
        .in (o_dec_imm),
        .out(o_wb_imm)
    );
    state_register U_WB_REG_PC_4 (
        .clk(clk),
        .rst(rst),
        .in (pc_alu_4),
        .out(o_wb_pc_alu_4)
    ...
    mux_5x1 U_WB_MUX (
        ...
        .in2    (o_wb_imm),         //from imm extend, for LUI
        .in3    (o_wb_pc_alu_imm),        //from pc + imm extend, for AUIPC
        .in4    (o_wb_pc_alu_4),          //from PC + 4, for JAL/JALR 
        .mux_sel(rfwd_src),
        .out_mux(rfwb_data)
    );
...
module program_counter (
    ...
    logic [31:0] pc_next_out, o_exe_pcnext, pc_rs1_mux_out;
    logic pc_next_sel;
